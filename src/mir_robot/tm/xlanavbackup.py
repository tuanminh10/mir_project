...
if os.path.isdir(_ros_sys_path) and _ros_sys_path not in sys.path:
    ...
    for mod_name in list(sys.modules.keys()):
        if any(mod_name.startswith(p) for p in ['geometry_msgs', 'nav_msgs', 'sensor_msgs',
                                                  'std_msgs', 'actionlib_msgs', 'tf2_msgs']):
...
STOP_DISTANCE = 1.3 # Robot dừng cách người 1.3m để an toàn
...
class MirNavigator:
    def __init__(self, ip="192.168.0.177"):
        ...
        # Đường dẫn tới script helper chạy trên Python 3.8 hệ thống
        self._helper_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "send_goal_helper.py")
    ...
    def ensure_ready(self):
        try:
            ...
            requests.put(f"http://{self.ip}/api/v2.0.0/status", headers=h, json={"clear_error": True, "state_id": 3}, timeout=1)
        except: pass
    ...
    def cancel_all(self):
        ...
        try:
            auth = "Basic YWRtaW46OGM2OTc2ZTViNTQxMDQxNWJkZTkwOGJkNGRlZTE1ZGZiMTY3YTljODczZmM0YmI4YTgxZjZmMmFiNDQ4YTkxOA=="
            h = {"Content-Type": "application/json", "Authorization": auth}
            requests.put(f"http://{self.ip}/api/v2.0.0/status", headers=h, json={"state_id": 4}, timeout=1)
    ...
    def send_goal(self, goal_x, goal_y, goal_yaw=0.0):
        """Gửi goal bằng cách spawn subprocess Python 3.8 hệ thống chạy ActionLib"""
        self.ensure_ready()
        try:
            # Dùng /usr/bin/python3 (Python 3.8 hệ thống - KHÔNG phải venv 3.9)
            # Giống hệt cách hung.py kết nối tới move_base
            ...
            # Đọc output trong thread riêng để không block
            def _log_output():
...
class tracking_loop:
                ...
                for i, box in enumerate(boxes):
                    ...
                    if track_id != -1 and self.locked_target_id is None:
                        ...
                        if self.open5_confirm_count.get(track_id, 0) >= 3:
                            ...
                            if hold_time >= 5.0:
                                ...
                                # mir_tts.speak_on_mir("Mục tiêu đã được xác nhận. Vui lòng giữ nguyên vị trí trong vòng 2 giây.")
                    ...
                    if track_id != -1 and track_id == self.locked_target_id:
                        ...
                        if self.fist_confirm_count > 3:
                            ...
                            if ho_time >= 5.0:
                                self.nav.cancel_all() # Phanh khẩn cấp
                                self.locked_target_id = None
                                self.robot_state = "IDLE"
                                global goal_pose
                                goal_pose = None
                                signal_bus.status_update.emit(f"Trạng thái: Đang theo dõi người dùng")
                                # mir_tts.speak_on_mir("Hủy lệnh di chuyển thành công.")

                    # Cập nhật Giao diện Box (luôn vẽ cho mọi người để không tưởng là bị đơ)
                    is_too_close = d_ngang_m < 1.0
                    
                    if self.locked_target_id is not None:
                        if track_id == self.locked_target_id:
                            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 255), 3) # Màu vàng Locked
                            ...
                            # Vẽ những người khác nhạt màu hơn để phân biệt
    ...
    def acquire_coords_and_navigate(self, distance_m, x1, y1, x2, y2):
        # KHÔNG GỌI self.depth_frame_cache() Ở ĐÂY để tránh crash luồng camera! 
        # (hàm get_person_relative_position_m vốn không dùng tham số depth_frame)
        ...
        time.sleep(2) # Thu thập tọa độ 2 giây

        # Quy trình chuyển đổi Lượng Giác TF từ (base_link) sang Absolute /map
        camera_offset_x = 0.475  # Robot mir origin is inside
        forward_m, left_m = rel[0] - camera_offset_x, rel[1]
        
        global tf_listener, goal_pose
        if tf_listener is not None:
            try:
                # 1. Fake 1 coordinate PointStamped ở base_link
                msg = PointStamped()
                ...
                # 3. Lệch ra khoảng an toàn (STOP_DISTANCE)
                if dist < 1.0:
                    signal_bus.status_update.emit(f"Mục tiêu quá sát (<1m). Hủy lệnh.")
                    self.robot_state = "IDLE"
                    self.locked_target_id = None
                    return

                ux, uy = dx/dist, dy/dist
                final_goal_x = target_x - ux * STOP_DISTANCE
                final_goal_y = target_y - uy * STOP_DISTANCE
                final_yaw = math.atan2(dy, dx)
                goal_pose = (final_goal_x, final_goal_y) # Để vẽ lên map xanh
                
                signal_bus.status_update.emit(f"Robot đang tiếp cận Mục Tiêu! (Cây cờ xanh)")
                # mir_tts.speak_on_mir("Bắt đầu tự động quét tọa độ và lộ trình di chuyển tới đối tượng.")
                
                # 4. Phát lệnh lái tự động thông qua Proportional Controller (/cmd_vel)
                self.nav.send_goal_cmd_vel(final_goal_x, final_goal_y)

            except Exception as e:
                signal_bus.status_update.emit(f"Lỗi tính toán không gian Map (TF)")
                self.robot_state = "IDLE"; self.locked_target_id = None
...
if __name__ == "__main__":
    ...
    rospy.loginfo("[Map] Đã đăng ký subscriber /map")
