...
class MapLabel(QLabel):
    ...
    def mouseReleaseEvent(self, event):
        if self.map_info is None or self.pixmap() is None: return
        
        # Lấy kích thước QLabel và QPixmap
        label_w = self.width()
        label_h = self.height()
        pixmap_w = self.pixmap().width()
        pixmap_h = self.pixmap().height()
        
        # Tính toán offset do căn giữa (Qt.AlignCenter)
        offset_x = (label_w - pixmap_w) / 2.0
        offset_y = (label_h - pixmap_h) / 2.0
        
        # Tính tọa độ thực trên ảnh map
        px = int(event.x() - offset_x)
        py = int(event.y() - offset_y)
        
        if not (0 <= px < pixmap_w and 0 <= py < pixmap_h):
            return
                            ...
                            if track_id in self.hand_raise_start:
                                ...
                                if hold_time >= 2.0:
                                    ...
                                    if rel is not None:
                                        z_opt = rel[2]
                                        x_opt = -rel[1]
                                        d_forward = z_opt * 0.95
                                        rel_X = -x_opt
                                        
                                        msg = PointStamped()
                                        msg.header.stamp = rospy.Time(0)
                                        msg.header.frame_id = "base_link"
                                        msg.point.x = d_forward + 0.475
                                        msg.point.y = rel_X
                                        msg.point.z = 0.0
...
class TestPCApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Tắt cảnh báo TF_REPEATED_DATA rác màn hình an toàn qua rosservice
        def mute_tf():
            time.sleep(1.5) # Đợi node đăng ký xong với ROS Master
            os.system(f"rosservice call {node_name}/set_logger_level 'ros.tf2' 'error' > /dev/null 2>&1")
            
        threading.Thread(target=mute_tf, daemon=True).start()
    ...
    def calculate_hybrid_safe_goal(self, target_x, target_y, obs_pt_map=None):
        ...
        if free_start_step is not None:
            ...
            # SỬA LỖI ĐỖ XA: Thu hẹp khoảng cách để robot tiến sát khách hàng hơn
            # Khoảng cách tâm xe (base_link) đến người: 0.55m đến 0.65m
            # Xe dài 0.89m (tâm đến mũi xe là 0.445m), nên mũi xe sẽ cách người khoảng 10-20cm!
        ...
        print(f"🚀 [NAV] Bắn lệnh tới MiR Fleet / MoveBase! (ĐÃ VÔ HIỆU HÓA TRONG FILE TEST NÀY)")
        
        # Bắt đầu theo dõi hành trình để thông báo khi tới nơi (Mô phỏng)
        self.current_goal = (goal_w_x, goal_w_y)
        self.is_moving = False  # Không di chuyển thật
        
        # Không gọi API hay WebSockets
