...
# Chuyển đổi siêu tốc toàn bộ thư viện qua môi trường ảo Python 3.9 (đã cài PyTorch 2.6)
# ==============================================================================
...
class MainControlMinimal:
    ...
    def on_web_order_received(self, msg):
        try:
            ...
            if "danh_sach_mon" in order_data:
                for mon in order_data["danh_sach_mon"]:
                    mon_lower = mon.lower()
                    if "coca" in mon_lower: coca_qty += 1
                    if "lavie" in mon_lower: lavie_qty += 1
    ...
    def verify_items_with_ai(self, expected_coca, expected_lavie):
        ...
        # Biến đếm thời gian liên tục đủ đồ
        continuous_success_start = None
        REQUIRED_SUCCESS_DURATION = 10.0 # Bắt buộc phải duy trì đủ đồ trong 10 giây liên tục
        ...
        while (rospy.Time.now() - start_time) < timeout:
            ...
            results = self.yolo_model.track(frame, persist=True, stream=False, conf=0.40, iou=0.45, imgsz=640, verbose=False)
            ...
            # Kiểm tra xem frame hiện tại có ĐỦ ĐỒ hay không
            is_enough_now = False
            ...
            # Xử lý Logic thời gian chờ 10 giây
            if is_enough_now:
                if continuous_success_start is None:
                    continuous_success_start = time.time()
                
                elapsed_time = time.time() - continuous_success_start
                remain_time = max(0, REQUIRED_SUCCESS_DURATION - elapsed_time)
                
                # Hiển thị thanh đếm ngược chữ xanh
                cv2.putText(annotated_frame, f"Giu nguyen: {remain_time:.1f}s", (20, 140), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 3, cv2.LINE_AA)
                            
                # Nếu đã duy trì liên tục quá 10 giây -> PASS
                if elapsed_time >= REQUIRED_SUCCESS_DURATION: 
                    rospy.loginfo("✅ [AI] Kiểm tra: ĐÃ ĐỦ ĐỒ TRONG 10S! Bắt đầu hành trình chạy.")
                    cap.release()
                    cv2.destroyAllWindows()
                    return True
            else:
                # Nếu bị thiếu hoặc rớt frame -> Bắt đầu đếm lại từ đầu
                continuous_success_start = None
                cv2.putText(annotated_frame, "THIEU DO - DEM LAI", (20, 140), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3, cv2.LINE_AA)

            # Hiển thị số lượng lên màn hình
            cv2.putText(annotated_frame, f"Coca: {count_coca}/{expected_coca}", (20, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3, cv2.LINE_AA)
            cv2.putText(annotated_frame, f"Lavie: {count_lavie}/{expected_lavie}", (20, 95), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 3, cv2.LINE_AA)
            
            cv2.imshow("Robot AI - Kiem tra Do An", annotated_frame)
            cv2.waitKey(1)
    ...
    def wait_until_items_taken(self):
        rospy.loginfo(f"📸 [AI] Đang mở Camera kiểm tra khách lấy đồ. Yêu cầu: Khay trống trong 5s liên tục...")
        cap = cv2.VideoCapture(0)
        
        if not cap.isOpened():
            rospy.logerr("❌ AI Không thể mở Camera!")
            self.wait_event.wait(timeout=10.0)
            return False
        ...
        start_time = rospy.Time.now()
        timeout = rospy.Duration(60.0) # Đợi tối đa 60 giây cho khách lấy đồ
        
        continuous_empty_start = None
        REQUIRED_EMPTY_DURATION = 5.0
        ...
        while (rospy.Time.now() - start_time) < timeout:
            ...
            is_empty = (count_coca == 0 and count_lavie == 0)

            if is_empty:
                if continuous_empty_start is None:
                    continuous_empty_start = time.time()
                
                elapsed_time = time.time() - continuous_empty_start
                remain_time = max(0, REQUIRED_EMPTY_DURATION - elapsed_time)
                
                cv2.putText(annotated_frame, f"Da xac nhan: {remain_time:.1f}s", (20, 140), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 3, cv2.LINE_AA)
                            
                if elapsed_time >= REQUIRED_EMPTY_DURATION: 
                    rospy.loginfo("✅ [AI] Khách đã lấy hết đồ!")
                    cap.release()
                    cv2.destroyAllWindows()
                    return True
            else:
                continuous_empty_start = None
                cv2.putText(annotated_frame, "DANG CHO LAY DO", (20, 140), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3, cv2.LINE_AA)

            cv2.putText(annotated_frame, f"Coca tren khay: {count_coca}", (20, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3, cv2.LINE_AA)
            cv2.putText(annotated_frame, f"Lavie tren khay: {count_lavie}", (20, 95), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 3, cv2.LINE_AA)
            
            cv2.imshow("Robot AI - Cho Khach Lay Do", annotated_frame)
            cv2.waitKey(1)
            
            # Cho phép ngắt sớm nếu khách ấn nút xác nhận trên web/nút bấm cứng
            if self.wait_event.is_set():
                rospy.loginfo("[Worker] Sự kiện wait_event được kích hoạt. Thoát AI kiểm tra đồ.")
                break
    ...
    def execute_task(self, task, cancel_event=None):
        ...
        if task_type == "GUEST_CALL":
            ...
            rospy.loginfo(f"[Worker] Đã giao xong đồ cho '{target}'. Chờ khách lấy đồ xuống...")
            mir_tts.speak_on_mir("Đây là món ăn quý khách đã order, xin mời quý khách nhận món ăn.")
            
            # AI kiểm tra khay trống trong vòng 5 giây liên tục
            if self.yolo_model:
                self.wait_until_items_taken()
            else:
                self.wait_event.wait(timeout=self.WAIT_TIME_DELIVER)
                
            mir_tts.speak_on_mir("Cảm ơn quý khách, chúc quý khách có bữa ăn ngon miệng.")
            rospy.sleep(3) # Chờ phát xong âm thanh
