...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        while self._run_flag:
                ...
                for i, box in enumerate(boxes):
                        ...
                        if self.open5_confirm_count.get(track_id, 0) >= 2:
                            if track_id in self.hand_raise_start:
                                ...
                                if hold_time >= 2.0:
                                    ...
                                    # Không lưu CSV ở bài TN2 này
                                    print(f"🎯 [TN2] Đã khóa mục tiêu ID {track_id} thành công!")
                                    ...
                                    try:
                                        ...
                                        # [TN2] VÔ HIỆU HÓA DI CHUYỂN ĐỂ CHỈ ĐO ĐẠC VÀ TEST NHẬN DIỆN!
                                        self.target_locked_signal.emit(pt.point.x, pt.point.y, None) 
                                        print(f"[TN2] Đã khóa đo đạc xong! (LOCKED) - Tọa độ Map: X={pt.point.x:.2f}, Y={pt.point.y:.2f}")
                                        print("[TN2] => BẤM PHÍM 'SPACE' TRÊN BÀN PHÍM ĐỂ ĐO LẦN TIẾP THEO!")
                                        self.robot_state = "LOCKED"
                                        self.locked_map_x = pt.point.x
                                        self.locked_map_y = pt.point.y
                                        # Vẫn giữ nguyên self.locked_target_id để nó không tự động quét lại
                                    except Exception as e:
                                        print(f"Lỗi TF (bỏ qua vì đang test TN2): {e}")
                                        self.robot_state = "LOCKED"
                                        self.locked_map_x = None
                                        self.locked_map_y = None
                    ...
                    if self.locked_target_id is not None:
                        ...
                        if is_raising:
                            ...
                            cv2.putText(annotated_frame, f"ID:{track_id} GIO TAY!", (int(x1), int(y1)-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                            
                            # Vẽ khung xương 2 cánh tay để debug
                            if keypoints is not None:
                                kp = kpts_xy[i]
                                cf = kpts_conf[i]
                                for pair in [(5,7), (7,9), (6,8), (8,10)]:
                                    if cf[pair[0]] > 0.25 and cf[pair[1]] > 0.25:
                                        pt1 = (int(kp[pair[0]][0]), int(kp[pair[0]][1]))
                                        pt2 = (int(kp[pair[1]][0]), int(kp[pair[1]][1]))
                                        cv2.line(annotated_frame, pt1, pt2, (0, 255, 0), 2)
                                        cv2.circle(annotated_frame, pt1, 4, (0, 0, 255), -1)
                                        cv2.circle(annotated_frame, pt2, 4, (0, 0, 255), -1)
                        elif not is_invalid:
            ...
            if self.locked_target_id is not None and getattr(self, 'locked_map_x', None) is not None:
                cv2.putText(annotated_frame, f"CUSTOMER COORD: X={self.locked_map_x:.2f}, Y={self.locked_map_y:.2f}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
...
class MainApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.video_thread.target_locked_signal.connect(self.on_target_locked)
        
        # [TN2] ÉP LUÔN LUÔN QUÉT TAY (Không cần qua bước ấn IoT hay GUEST_CALL)
        ...
        print("\n" + "="*60)
        print("🚀 [TN2] GIAO DIỆN ĐO ĐẠC ĐÃ SẴN SÀNG!")
    ...
    def on_target_locked(self, wx, wy, obs_pt):
        self.map_label.set_target(wx, wy)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Space:
            if hasattr(self, 'video_thread'):
                ...
                self.video_thread.locked_map_x = None
                self.video_thread.locked_map_y = None
                self.map_label.clear_target()
