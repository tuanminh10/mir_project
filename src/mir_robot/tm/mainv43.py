...
class MainApp(QMainWindow):
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start > timeout:
                        ...
                        msg_parts = []
                        if check_empty:
                            if coca > 0: msg_parts.append(f"{coca} Coca")
                            if lavie > 0: msg_parts.append(f"{lavie} Lavie")
                            ...
                            if coca < ec: msg_parts.append(f"thiếu {ec - coca} Coca")
                            if lavie < el: msg_parts.append(f"thiếu {el - lavie} Lavie")
                            speak_msg = "Hình như " + " và ".join(msg_parts) + ", xin bếp kiểm tra lại."
                            print(f"[VERIFY TRAY] ❌ Quá {timeout}s chưa đủ đồ: {speak_msg}")
                            threading.Thread(target=mir_tts.speak_on_mir, args=(speak_msg,)).start()
    ...
    def calculate_hybrid_safe_goal(self, target_x, target_y, obs_pt_map=None, min_dist_m=0.95, attempt=0, mode="ORDER"):
        ...
        # Giáo sư Antigravity: Tăng mạnh inflate_m lên 0.40m (thay vì 0.25m) để bao trùm luôn cả 
        # phần khung ghế sắt bên dưới mà LiDAR không nhìn thấy (do quá thấp).
        # Cứ thấy chân người là tự hiểu xung quanh có cái ghế bán kính 40cm!
        inflate_m = 0.40
        ...
        theta_raw_left = theta_open + math.radians(60)
        theta_raw_right = theta_open - math.radians(60)
        global_angles = [60, 150, -60, -150]
        ...
        # =========================================================================
        # ORDER MODE: SỬ DỤNG TIA 60 ĐỘ ĐỂ ĐỖ CHÉO
        # =========================================================================
        ...
        if step_left is not None and step_right is not None:
            ...
            print("[SMART NAV] ⚠️ Không tìm được footprint hợp lệ ở các góc 60 độ; dùng khoảng cách tối thiểu để tránh treo.")
