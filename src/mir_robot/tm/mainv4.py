    ...
    def execute_task(self, task, cancel_event=None):
        ...
        if ttype == "GUEST_CALL":
            ...
            if self.scanning_event.wait(timeout=20.0):
                ...
                # Chờ vô thời hạn cho đến khi khách order trên Web (hàm on_web_order gọi self.wait_event.set())
            ...
            for i in range(3): # Chờ tối đa 3 lần x 20 giây = 60s
                ...
                if is_empty:
                    break
                if i < 2:
                    mir_tts.speak_on_mir("Quý khách vui lòng lấy hết đồ uống trên khay để robot tiếp tục làm việc.")
            
            # Khách lấy xong hoặc hết giờ, ngẩng camera lên lại
            if self.servo: self.servo.set_angle(95)
